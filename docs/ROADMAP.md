...
- [ ] **Merge `ntvdmhost`+`ntvdmex`** into one windowed host: route INT 21h console output + INT 10h
  through the video VDD + present_ddraw so a real DOS prompt paints in the Luna window.
- [ ] Video VDD: graphics modes — 13h (linear 320×200×256) + DAC, then VESA VBE 2.0 banked.
