...
#
# Hand-assembled (org 0x100); see the byte map in git history of this file.
#   mov ax,0013; int10 | DAC: out 3C8,0; loop 256x out 3C9 R,(R>>1),(63-R)
#   mov es,A000; fill 64000 bytes [di]=al(++) | mov ah,0;int16 | mov ax,3;int10
#   | mov ax,4C00; int21
#
set -euo pipefail
...
printf '\xB8\x00\xA0\x8E\xC0\x31\xFF\xB9\x40\xFA\x31\xC0'   >> "$out"    # es=A000; di=0; cx=64000; al=0
printf '\x26\x88\x05\x47\xFE\xC0\xE2\xF8'                   >> "$out"    # [es:di]=al; di++; al++; loop
...
[ "$sz" -eq 79 ] || { echo "ERROR: expected 79 bytes, got $sz" >&2; exit 1; }
