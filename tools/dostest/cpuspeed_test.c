...
int main(void)
{
    ...
    CHECK(cpuspeed_duty_bp(4, 0) == 10000, "a zero reference cannot throttle (never divide by it)");
    ...
    {   unsigned bp = cpuspeed_duty_bp(4, 700);
        CHECK(bp >= 460 && bp <= 480, "33 MHz against a 700 MHz reference is ~4.7% duty"); }

    {   int mono = 1;
        for (i = 2; i < CPUSPEED_COUNT; ++i)
            if (cpuspeed_duty_bp(i, 700) >= cpuspeed_duty_bp(i - 1, 700)) mono = 0;
        CHECK(mono, "a slower setting is always a smaller duty -- monotonic, no ties"); }
    ...
    CHECK(cpuspeed_duty_bp(6, 4000000u) >= 1, "an absurd reference still leaves the guest some time");
    ...
    CHECK(cpuspeed_duty_bp(1, 100) == 10000, "200 MHz on a 100 MHz reference clamps to flat out");
    ...
    {   unsigned got = cpuspeed_delivered_mhz(6, 3661, 3500);
        CHECK(got > CPUSPEED_MHZ[6],
    ...
    {   unsigned long ips = cpuspeed_ips(4);
    ...
    {   cpuspeed_pace p; int ms;
        memset(&p, 0, sizeof p);
        ms = cpuspeed_charge(&p, 1000000ul, cpuspeed_ips(4), 5000ll);
    ...
    {   cpuspeed_pace p; int k, ms, tot = 0;
        ...
        for (k = 0; k < 10; ++k) {
            ...
            ms = cpuspeed_charge(&p, 3300ul, cpuspeed_ips(4), 0ll);
            tot += ms;
        }
    ...
    {   cpuspeed_pace p; int ms;
        memset(&p, 0, sizeof p);
        ms = cpuspeed_charge(&p, 100ul, cpuspeed_ips(4), 500000ll);
        CHECK(ms == 0 && p.owed_us == 0, "a slice slower than its budget sleeps 0 and banks nothing");
        ms = cpuspeed_charge(&p, 1000000ul, cpuspeed_ips(4), 0ll);
    ...
    {   cpuspeed_pace p; int ms;
        memset(&p, 0, sizeof p);
        ms = cpuspeed_charge(&p, 100000000ul, cpuspeed_ips(6), 0ll);
