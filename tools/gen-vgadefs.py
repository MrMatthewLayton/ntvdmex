...
def parse(path):
    ...
    for line in open(path):
        ...
        if m:
            ...
            order.append(cur)
            modes[cur] = {"ac": [int(x, 16) for x in m.group(2).split()],
                          "dac": [], "crtc": []}
            continue
        m = re.match(r"CRTC=(.*)", line)
        if m and cur is not None:
            modes[cur]["crtc"] = [int(x, 16) for x in m.group(1).split()]
            continue
    ...
    for k, v in modes.items():
        if len(v["ac"]) != 16 or len(v["dac"]) != 256 or len(v["crtc"]) != 25:
            sys.exit("mode %02X: got %d AC, %d DAC and %d CRTC entries, want 16, 256 and 25"
                     % (k, len(v["ac"]), len(v["dac"]), len(v["crtc"])))
    if not modes:
...
def main():
    ...
    ac_tabs, dac_tabs, crtc_tabs = [], [], []
    ac_of, dac_of, crtc_of = {}, {}, {}
    for m in order:
        a = tuple(modes[m]["ac"])
        d = tuple(modes[m]["dac"])
        c = tuple(modes[m]["crtc"])
        if a not in ac_of:
            ac_of[a] = len(ac_tabs)
            ac_tabs.append(a)
        if d not in dac_of:
    ...
    w.append("};")
    w.append("")
    w.append("/* CRTC registers 00..18, per mode. A mode set reprograms the whole CRTC;")
    w.append("   what the host acts on is 0x13 (Offset, bytes per scan line in words)")
    w.append("   and 0x0C/0x0D (start address), because those are what the renderer")
    w.append("   reads. The rest is carried so the next question about CRT geometry can")
    w.append("   be answered from a measurement instead of another run. */")
    ...
    for i, c in enumerate(crtc_tabs):
        ...
        w.append("    /* %d: modes %s -- offset=0x%02X */"
                 % (i, ", ".join("%02Xh" % m for m in ms), c[0x13]))
        w.append("    { " + ", ".join("0x%02X" % v for v in c) + " },")
    w.append("};")
    w.append("")
    ...
    w.append("    { " + ", ".join("%d" % crtc_of[tuple(modes[m]["crtc"])] for m in order) + " },")
    w.append("};")
